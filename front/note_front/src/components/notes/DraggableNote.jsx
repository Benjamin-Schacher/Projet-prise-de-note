...
export default function DraggableNote({ id, title, contentPreview, position, onSize, onOpen }) {
  ...
  // fusionne la ref useDraggable et la ref setNodeRef
  const setRefs = (node) => {
    setNodeRef(node);
  ...
  const outerStyle = {
    ...
    left: position.x,
    top: position.y,
    transform: transform //animation pendant le drag
  ...
  const innerStyle = { // ombre du drag
    boxShadow: isDragging ? '0 10px 30px rgba(0,0,0,0.3)' : '0 5px 10px rgba(0,0,0,0.1)',
    display: 'inline-block',
    transition: isDragging ? 'none' : 'transform 0.3s ease',
    animation: isDragging ? 'wiggle-rotate 1s infinite' : 'none',//animation de secousse
  };

  React.useEffect(() => {//mesure la note et revérifie lorsqu'elle change de taille
  ...
  return ( //deux div pour la position et l'animation du drag
